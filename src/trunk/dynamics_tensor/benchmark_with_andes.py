...
from VeraGridEngine.Utils.MultiLinear.to_diffblock import block2diffblock
...
def perform_andes_simulation_for_kundur_ieee_no_shunt():
    ...
    timings = {}
    timings['power_flow_time'] = powerflow_time
    timings['compiling_time'] = compiling_time
    timings['simulation_time'] = simulation_time

    return andes_simulation_df, timings

# perform simulation in VeraGrid
def perform_diffblock_simulation_for_kundur_ieee_no_shunt():
    ...
    if run_powerflow := False:
        ...
        with open("pf_results_kundur.pkl", "wb") as f:
            pickle.dump(res, f)
    else:
        with open("pf_results_kundur.pkl", "rb") as f:
    ...
    ss = block2diffblock(ss)

    dx0_mapping = {diffvar: Const(0) for diffvar in ss.diff_vars}
    params_mapping = {}

    slv = DiffBlockSolver(ss, t)
    ...
    dx0 = slv.build_init_diffvars_vector(dx0_mapping)
    vars_in_order = slv.sort_vars_from_uid(init_guess)

    start_simulation = time.time()

    t, y, timings = slv.simulate(
        t0=0, 
        t_end=20.0,
        h=0.001,
        x0=x0,
    ...
    timings['power_flow_time'] = power_flow_time
    timings['compiling_time'] = compiling_time
    timings['simulation_time'] = simulation_time
    return veragrid_simulation_df, timings
...
def andes_veragrid_simulation_results(tol=1e-3):
    ...
    timings : dict
        Dictionary with computation and simulation times for ANDES and VeraGrid.
    """
    # Run simulations
    veragrid_timings = {}
    veragrid_sim_results, veragrid_timings = perform_diffblock_simulation_for_kundur_ieee_no_shunt()
    andes_sim_results, andes_timings = perform_andes_simulation_for_kundur_ieee_no_shunt()

    # Store timing information
    timings = {
        "ANDES": andes_timings,
        "VeraGrid": veragrid_timings
    }
    ...
    return success, report, timings


# Example usage
success, report, timings = andes_veragrid_simulation_results()
